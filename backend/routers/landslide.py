...
@router.post("/api/landslide/autocollect")
async def run_landslide_autocollect():
    """
    Auto-Collect: Downloads terrain data + RF labels from predefined
    mountainous locations and batch-trains the local U-Net model.
    """
    try:
        logger.info("Landslide Auto-Collect request received")
        
        loop = asyncio.get_event_loop()
        from backend.services.analysis.dl_landslide import auto_collect_landslide
        
        result = await loop.run_in_executor(
            None,
            lambda: auto_collect_landslide(num_locations=5),
        )
        
        return result
    except Exception as e:
        logger.error(f"Landslide Auto-Collect failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
